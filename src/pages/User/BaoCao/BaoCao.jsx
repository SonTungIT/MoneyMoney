...
function BaoCao() {
    ...
    const fetchTotalByMonth = (category, date) => {
        ...
        fetch(
            `https://moneymoney.azurewebsites.net/api/v1/money-money/users/${category}/total-by-month?date=${year}%2F${month}%2F01`,
            requestOptions,
        )
    ...
    const fetchTotalProfit = (date) => {
        ...
        Promise.all([
            fetch(
                `https://moneymoney.azurewebsites.net/api/v1/money-money/users/profits/total-by-month?date=${year}%2F${month}%2F01`,
                requestOptions,
            ).then((response) => response.json()),
            fetch(
                `https://moneymoney.azurewebsites.net/api/v1/money-money/users/profits/starting-balance/${month}/${year}`,
                requestOptions,
            ).then((response) => response.text()),
            fetch(
                `https://moneymoney.azurewebsites.net/api/v1/money-money/users/profits/ending-balance/${month}/${year}`,
                requestOptions,
            ).then((response) => response.text()),
        ])
